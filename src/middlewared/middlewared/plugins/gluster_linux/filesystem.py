...
class GlusterFilesystemService(Service):
    ...
    * add wrapper for handle.fts_open() method to iterate fs path
    ...
    def copy_tree(self, job, data):
        ...
        dst_vol = glfs.init_volume_mount(data['src_volume_name'], data['gluster-volume-options'])

        src_hdl = self.get_object_handle(src_vol, data['src_uuid'])
        dst_hdl = self.get_object_handle(src_vol, data['dst_uuid'])
        ...
        for idx, entry in enumerate(src_hdl.fts_open()):
            ...
            if entry.file_type == 'DIRECTORY':
                dst_hdl_lst.append(dst.mkdir(entry.name, new_mode))
            elif entry.file_type == 'FILE':
                hdl = dst.create(entry.name, os.O_RDWR, mode=new_mode)
                hdl.open(os.O_RDWR).pwrite(entry.handle.contents(), 0)
