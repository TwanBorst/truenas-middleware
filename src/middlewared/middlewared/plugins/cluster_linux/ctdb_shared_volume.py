...
class CtdbSharedVolumeService(Service):
    ...
    def update(self, job, data):
        ...
        """
        volume, uuid = data['name'].rsplit(':', 1)
    ...
    def migrate(self, job, data):
        ...
        self.middleware.call_sync('gluster.localevents.send', {
            'event': 'METADATA_VOL_UPDATE',
            'name': f'{data["name"]}:{dst_uuid}',
            'forward': True
        })
        ...
        while remaining > 0:
            ...
            for file_name in contents:
                ...
                peer_uuid = file_name.strip('_UPDATE_IN_PROGRESS')
                job.set_progress(50, f'Waiting for peer [{peer_uuid[1:]}] to finish update.')
                sleep(10)
                remaining -= 10
